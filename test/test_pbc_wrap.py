...
def test_pbc_wrap_coords():
    ...
    np.allclose(coords_frac_wrap, crys.pbc_wrap_coords(coords_frac_orig))
    ...
    np.allclose(coords_frac_orig[plus == 0],      coords_frac_wrap[plus == 0])
    np.allclose(coords_frac_orig[plus == -1] + 1, coords_frac_wrap[plus == -1])
    np.allclose(coords_frac_orig[plus == 1] - 1,  coords_frac_wrap[plus == 1])
    # the PBC wrapping must restore coords_frac_wrap_ref 
    np.allclose(coords_frac_wrap_ref, coords_frac_wrap)

    # pbc only in x-y, not z
    coords_frac_wrap = crys.pbc_wrap_coords(coords_frac_orig, mask=[True,True,False], xyz_axis=-1)
    np.allclose(coords_frac_orig[...,2], coords_frac_wrap[...,2])
...
def test_pbc_wrap():
    ...
    np.allclose(coords_frac_orig[plus == 0],      tr_wrap.coords_frac[plus == 0])
    np.allclose(coords_frac_orig[plus == -1] + 1, tr_wrap.coords_frac[plus == -1])
    np.allclose(coords_frac_orig[plus == 1] - 1,  tr_wrap.coords_frac[plus == 1])
    np.allclose(coords_frac_wrap, tr_wrap.coords_frac)
    np.allclose(coords_frac_wrap, tr_wrap.coords/2.0)
