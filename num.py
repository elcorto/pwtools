...
def rot_matrix(alpha,beta,gamma, rot=1):
    """Rotation matrix for rotating a vector w.r.t. a coord sys (``rot=1``) or
    vice versa (``rot=-1``).
    
    Parameters
    ----------
    alpha, beta, gamma : float
        Angles in degree for rotation about x,y,z. Use 0 for no rotation.
    rot : float
        1: rotation of vector w.r.t. coord sys
       -1: rotation of coord sys w.r.t. vector
    
    Examples
    --------
    >>> # rotate [1,0,0] 45 deg about z axis into xy plane
    >>> R=num.rot_matrix(0,0,45)
    >>> dot(R, array([1,0,0]))
    array([ 0.70710678,  0.70710678,  0.        ])
    >>> # apply to times is the same as rot_matrix(0,0,90)
    >>> dot(R,dot(R, array([1,0,0])))
    array([  2.22044605e-16,   1.00000000e+00,   0.00000000e+00])
    >>> # rotate many vectors
    >>> dot(array([[1,0,0],[2,0,0]]), R.T)
    array([[ 0.70710678,  0.70710678,  0.        ],
    ...
    ry = np.array([[ cos_b,      0,  sin_b],
                   [     0,      1,      0],
                   [-sin_b,      0,  cos_b]])*1.0
    
    rz = np.array([[ cos_c, -sin_c,      0],
                   [ sin_c,  cos_c,      0],
                   [     0,      0,      1]])*1.0
