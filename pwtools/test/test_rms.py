...
def test_rms():
    ...
    # Test if rms() works.
    r3_rms_all1 = num.rms(arr3, nitems='all')
    r3_rms_natoms1 = num.rms(arr3, nitems=natoms)
    ...
    r1_3d = num.rms3d(arr1, axis=0, nitems='all')
    r2_3d = num.rms3d(arr2, axis=1, nitems='all')
    r3_3d = num.rms3d(arr3, axis=2, nitems='all')
    r1_loop = np.empty((nstep,), dtype=float)
    r2_loop = np.empty((nstep,), dtype=float)
    r3_loop = np.empty((nstep,), dtype=float)
    for k in range(nstep):
        r1_loop[k] = num.rms(arr1[k,...], nitems='all')
        r2_loop[k] = num.rms(arr2[:,k,:], nitems='all')
        r3_loop[k] = num.rms(arr3[...,k], nitems='all')
    ...
    from_rms3d = num.rms3d(traj.coords - traj.coords[0,...][None,...],
