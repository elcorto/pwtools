...
singular. Then one should solve a regression problem instead (see below).
...
without regularization. You can do that by setting ``Rbf(..., r=None``) which
is also the default, since we don't like to set a specific value for
:math:`r>0` because that depends entirely on the data set. In
:class:`sklearn.kernel_ridge.KernelRidge` the switch to least squares is done
automatically when a singular :math:`\mathbf K` is detected. However, we
observed that in some cases this results in small numerical noise in the
solution, so test before using and maybe switch to regularization in that case.
...
:math:`p`, with :math:`r` constant and very small such as ``1e-11`` or
``r=None`` (least squares solver). See ``examples/rbf/overfit.py``.
...
``examples/rbf/crossval_map_p_r.py``) by plotting the CV score as function both.
This is the result for the Gaussian RBF.
