...
class PwMDOutputFile(TrajectoryFileParser, PwSCFOutputFile):
    ...
    def get_timestep(self):
        """Time step [tryd]."""
        cmd = r"grep -i 'time.*step' %s | sed -re \
...
# FIXME All Abinit* classes: Units not handled. 
# FIXME Abinit*MD* classes: Add get_timestep()
class AbinitSCFOutputFile(StructureFileParser):
    ...
    first value along timeaxis is returned.
    ...
    # 
    def __init__(self, *args, **kwds):
        raise NotImplementedError("don't use, fix units first, adapt tests")
        ...
        # grepped more than once (e.g. stress).
        self.timeaxis = Trajectory().timeaxis
    
    def dump(self, *args, **kwds):
        if kwds.has_key('slim'):
            slim = kwds['slim']
            kwds.pop('slim')
        else:
            slim = True
        if slim:
            ...
            if self.is_set_attr('_stress_raw'):
                del self._stress_raw
    ...
    # for MD-like, it greps the *last* printed block (the one in the
    #  summary)!
    ...
    def get_typat(self):
        ...
        if self.is_set_attr_lst(req):
            ...
            if match is None:
                ...
                return np.array(match.group(1).strip().split()).astype(int).tolist()
        else:
            return None
    ...
    def get_symbols(self):
        ...
        self.try_set_attr_lst(req)
        if self.is_set_attr_lst(req):
            idxs = (np.asarray(self.typat) - 1).astype(int)
...
# TODO: Add get_timestep()
# TODO: add velocity unit
class AbinitMDOutputFile(TrajectoryFileParser, AbinitSCFOutputFile):
    ...
    def get_stress(self):
        return self.raw_return('stress')

    def get_nstep(self):
        if self.check_set_attr('coords_frac'):
    ...
    def get_cryst_const(self):
        ...
        if self.is_set_attr_lst(req):
            ...
            if n1 != n2: 
                ...
                nn = n1
            cc = np.concatenate((self.lengths[-nn:,:], self.angles[-nn:,:]), 
                                 axis=1)
            return cc                                 
        else:
            return None
...
# XXX don't use, see AbinitSCFOutputFile
class AbinitVCMDOutputFile(AbinitMDOutputFile):
    ...
    def get_coords_frac(self):
        verbose("getting coords_frac")
        return self.raw_slice_get('coords_frac', 
                                   sl=np.s_[::2],
                                   axis=self.timeaxis)
    
    def get_forces(self):
        verbose("getting forces")
        return self.raw_slice_get('forces', 
                                   sl=np.s_[::2],
                                   axis=self.timeaxis)
    
    def get_velocity(self):
        verbose("getting velocity")
        return self.raw_slice_get('velocity', 
                                   sl=np.s_[::2],
                                   axis=self.timeaxis)
